...
export const useUserEvaluation = () => {
    ...
    datasets: [
      {
        ...
        data: [],
        borderWidth: 2,
        borderColor: ColorEnum.COLOR_KANRISHA,
        pointBorderColor: ColorEnum.COLOR_KANRISHA,
        pointRadius: 5,
        fill: false,
      },
      {
        ...
        backgroundColor: ColorEnum.COLOR_TRAINER,
        data: [],
        borderWidth: 2,
        borderColor: ColorEnum.COLOR_TRAINER,
        pointRadius: 5,
        pointBorderColor: ColorEnum.COLOR_TRAINER,
        fill: false,
      },
    ],
  })
  ...
  const totalCheckPoint = computed(() => {
    if (
      !memberStore?.lastEvaluationHistories?.check_points
        ?.length
    )
      return 0
    return (
  ...
  const chartValue = computed(() => {
    ...
    const physicalPoint =
      memberStore?.evaluationHistories.data
        .map((e) => {
          return e.check_points.reduce<number>(
    ...
    return {
      ...
      psychological_point: psychologicalPoint,
    }
  })
  ...
  const percentCompleted = computed(() => {
    if (
      ...
      return 0
    return (
      (totalCheckpointComplete.value /
        totalCheckPoint.value) *
      100
    )
  })
    ...
    ([newChartDate, newChartValue, newTotalValue]) => {
      ...
      if (newChartValue) {
        chartData.value.datasets[0].data =
          newChartValue.physical_point
        chartData.value.datasets[1].data =
          newChartValue.psychological_point
      } else {
        chartData.value.datasets[0].data = []
        chartData.value.datasets[1].data = []
      }

      optionChart.value.scales.y.max =
        newTotalValue / 2 + 20
    },
    {
      immediate: true,
