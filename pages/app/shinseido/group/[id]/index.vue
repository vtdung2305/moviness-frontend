...
const inforsFacility = computed(() => {
  return [
    {
      label: t('pages.group_detail.facility_name'),
      value:
        groupData.value?.facility?.name ||
        t('others.empty'),
      linkText: t('pages.group_detail.facility_link'),
      link: `/app/shinseido/facility/${groupData.value?.facility?.id}`,
    },
  ]
})
const facilityImage = computed(() => {
  ...
  return groupData.value?.facility?.avatar
    ? groupData.value?.facility?.url_avatar
    : imageDefault
})
        ...
        <div v-else class="group-detail laptop:w-3/4">
          <div class="group-detail__container">
            ...
            <CommonInfo
              :title="
                $t(
                  'pages.group_detail.facility_information'
                )
              "
              :src-img="facilityImage"
              :infors="inforsFacility"
              class="mt-59px relative"
            />
            <div
