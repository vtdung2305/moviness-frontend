...
const getValueVideoDetail = async () => {
  ...
  await videoStore.getVideoDetail(route.params.id as string)
  isLoading.value = false
}

const handleViewEditVideo = (id: number | string) => {
  const role = userStore.roleName
  router.push(`/app/${role}/videos/${id}/edit`)
}
      ...
      <template v-else>
        ...
        >
          <div class="desktop:w-3/4">
            ...
            <Button
              :is-disabled="
                !videoStore?.videoDetail?.can_edit
              "
              type="primary"
              class="mt-13px mb-23px"
              @click="
                handleViewEditVideo(
                  videoStore?.videoDetail?.id as number
                )
              "
            >
              {{ $t('pages.shinseido.video_detail.edit') }}
            </Button>
          </div>
        </PageSection>
      </template>
